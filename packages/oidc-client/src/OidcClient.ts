...
> extends EventEmitter<OidcClientEvents> {
        ...
        return {
            ...
            access_token:
                typeof input.access_token === "string"
                    ...
                    : undefined,
            id_token:
                typeof input.id_token === "string" ? input.id_token : undefined,
            refresh_token:
            ...
            token_type:
                typeof input.token_type === "string"
                    ? input.token_type
                    : undefined,
            expires_in: Number.isNaN(expiresIn ?? Number.NaN)
                ? undefined
                : expiresIn,
    ...
    async handleSigninCallback(url?: URL): Promise<OidcTokenResponse | null> {
        ...
        const idToken = this.readCallbackParam(
            queryParams,
            hashParams,
            "id_token",
        );
        if (accessToken || idToken) {
            const tokenResponse = this.normalizeTokenResponse({
                ...
                scope:
                    this.readCallbackParam(queryParams, hashParams, "scope") ??
                    undefined,
    ...
    private async registerClient(config: OidcConfiguration): Promise<string> {
        ...
        if (isNative) {
            ...
            const json =
                (await response.json()) as OidcClientRegistrationResponse;
            if (!json.client_id) {
        ...
        if (!res.ok) {
            throw new Error(
                `Failed dynamic client registration: ${res.status}`,
            );
        }
